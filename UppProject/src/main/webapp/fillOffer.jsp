...
<body>
	...
	<div id="container"
		...
		<h2
			style="text-align: center; padding-top: 10px; padding-bottom: 10px;">Ponuda</h2>
		<c:if test="${fn:length(formProperties) > 0}">

			<form name='f' action="<c:url value='/submitFilledOffer' />"
				method='POST'>
				...
				<fieldset>
					...
					<c:forEach var="formProperty" items="${formProperties}">
						...
						<c:if test="${formProperty.readable == true}">
							...
							<c:choose>
								<c:when test="${formProperty.id.equals('date')}">
									<input type="text"
